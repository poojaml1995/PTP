...
<body id="body-pd">
    ...
    <div class="heading h3 mx-3">
        <h3 class="mt-5">Product Details</h3>
    </div>
    <div class="container overflow-hidden d-flex flex-wrap p-0 mb-5">
        <?php
            ...
            while ($row = mysqli_fetch_array($query)) {
                ...
                <div class="card mt-1">
                    <div class="card_header">
                        <?php 
                        if (!empty($row["image_loc"])) { 
                            ?>
                            <img src="<?php echo htmlspecialchars($row["image_loc"]); ?>" alt="card_image" class="card_image" width="800">
                            <?php 
                        }
                        ?>
                    </div>
                    <div class="card_body">
                        <h5 class="card-title ptno mb-0"><strong>PT Round / Scheme No :</strong> <?php echo htmlspecialchars($row['product_id']); ?></h5>
                        <h5 class="card-title"><?php echo htmlspecialchars($row['product_name']); ?></h5>
                        <a href="./view_product.php?view_product_id=<?php echo htmlspecialchars($row['product_id']); ?>" class="btn btn-primary mb-3 w-50">View Products</a>
                    </div>
                </div>
                <?php
            }
        } else {
            echo '<div class="no-products-available"><strong><p>No products found</p></strong></div>';
        }
        ?>
    </div>
</div>
